...
class BookingCalendar extends DatabaseConnection
{
    public function buildCalendar($month, $year)
    {
        ...
        $stmt = $this->getConnection()->prepare("SELECT date, COUNT(*) as booked_count FROM bookings1 WHERE MONTH(date) = ? AND YEAR(date) = ? GROUP BY date");
    ...
    {
        $stmt = $this->getConnection()->prepare("SELECT * FROM bookings1 WHERE date = ?");
        $stmt->execute([$date]);
    ...
    {
        $stmt = $this->getConnection()->prepare("INSERT INTO bookings1 (name, email, phone, vehicle_model, vehicle_number, timeslot, date) VALUES (?, ?, ?, ?, ?, ?, ?)");
    ...
    public function getTimeslotStatus($date)
    {
        ...
        $stmt = $this->getConnection()->prepare("SELECT timeslot FROM bookings1 WHERE date = ?");
        $stmt->execute([$date]);
